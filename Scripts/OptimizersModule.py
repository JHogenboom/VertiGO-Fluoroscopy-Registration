...
class NL_opt_LN_NELDERMEAD_ORIGINEEL():
...
class NL_opt_LN_NELDERMEAD():
    ...
    def _set_bound_constraints(self, initial_parameters):
        ...
        for i in range(len(self.to_optimize)):
            print(' \n Bound constraints for ', self.param_sequence[self.to_optimize[i]], ': ', self.real_lb[i],
                  self.real_ub[i])
    ...
    def _optimize(self, parameters, verbose=False):
        ...
        # Normalize initial parameters if needed
        if self.Norm:
            parameters_to_optimize = list(np.divide(parameters_to_optimize - np.asarray(self.real_lb),
                                                    (np.asarray(self.real_ub) - np.asarray(self.real_lb))))

        # Run optimization
        #optimal_parameters = self.opt_LN_NELDERMEAD.optimize(parameters_to_optimize)
        optimal_parameters = np.array([np.deg2rad(0.0), np.deg2rad(0.0), np.deg2rad(8.0), 0.0, 0.0, 0.0], dtype = np.float64)
        # De-normalized optimal parameters if needed
        if self.Norm:
            optimal_parameters = list(
                np.multiply((np.asarray(self.real_ub) - np.asarray(self.real_lb)), optimal_parameters) + np.asarray(
                    self.real_lb))
