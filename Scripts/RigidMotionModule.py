...
def augment_matrix_coord(array):
    """
        Returns an augmented vector using numpy.ones

        :param numpy.ndarray array: empty array
        :return np.ndarray: augmented vector
    """
...
def get_rotation_mat_single_axis(axis, angle):
    """
        Computes the 3X3 rotation matrix relative to a single rotation of angle in radians
        Around the axis(string 'x', 'y', 'z') for a right handed CS
    """
...
def get_euler_zxy(rot_matrix):
    """
        retrieves ZXY Cardan sequence of angles from given rigid transformation matrix.
    """

    if rot_matrix[2, 1] != 1. and rot_matrix[2, 1] != -1.:
        euler_x_1 = np.arcsin(rot_matrix[2, 1])
        euler_y_1 = - np.arctan2(rot_matrix[2, 0] / np.cos(euler_x_1), rot_matrix[2, 2] / np.cos(euler_x_1))
        euler_z_1 = - np.arctan2(rot_matrix[0, 1] / np.cos(euler_x_1), rot_matrix[1, 1] / np.cos(euler_x_1))
        euler_x_2 = np.pi - np.arcsin(rot_matrix[2, 1])
        euler_y_2 = - np.arctan2(rot_matrix[2, 0] / np.cos(euler_x_1), rot_matrix[2, 2] / np.cos(euler_x_1))
        euler_z_2 = - np.arctan2(rot_matrix[0, 1] / np.cos(euler_x_1), rot_matrix[1, 1] / np.cos(euler_x_1))
